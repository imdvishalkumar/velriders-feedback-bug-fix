...
class CarHostVehicleDetailsController extends Controller
{
    ...
    {
        Log::info('HOST_DEBUG: getManufacturers called', ['params' => $request->all()]);
    ...
    {
        Log::info('HOST_DEBUG: getModels called', ['params' => $request->all()]);
        $typeId = $request->input('vehicle_type_id') ?? '';
    ...
    {
        Log::info('HOST_DEBUG: getYears called');
        $startYear = 2010;
        $currentYear = date('Y');
        $yearArr = [];
    ...
    {
        Log::info('HOST_DEBUG: getKmDriven called');
    ...
    {
        Log::info('HOST_DEBUG: getCarHostVehicles called', ['params' => $request->all()]);
    ...
    {
        Log::info('HOST_DEBUG: getCategories called');
    ...
    {
        Log::info('HOST_DEBUG: getVehicleDropdownValues called', ['params' => $request->all()]);
        $data = [];
        $validator = Validator::make($request->all(), [
    ...
    {
        Log::info('HOST_DEBUG: getVehicleDesc called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
    ...
    {
        Log::info('HOST_DEBUG: getParkingType called');
    ...
    {
        Log::info('HOST_DEBUG: getHoldVehicleDates called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
            'vehicle_id' => 'required|exists:vehicles,vehicle_id',
        ]);
        if ($validator->fails()) {
    ...
    {
        Log::info('HOST_DEBUG: getListingHours called');
    ...
    {
        Log::info('HOST_DEBUG: getVehicleFeatures called');
    ...
    {
        Log::info('HOST_DEBUG: getBookings called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
        ...
        if ($page !== null && $pageSize !== null) {
            ...
            if (isset($rentalBookings) && is_countable($rentalBookings) && count($rentalBookings)) {
                foreach ($rentalBookings as $key => $val) {
                    $showCustomer = ($val->status != 'pending') || (isset($val->pickup_date) && (Carbon::parse($val->pickup_date)->timestamp - $now->timestamp <= 3600));
                    if ($showCustomer && $val->customer != '') {
            ...
            $filtered = $rentalBookings->getCollection()->filter(function ($item) use ($oneHourAgo) {
    ...
    {
        Log::info('HOST_DEBUG: getBookingDetails called', ['booking_id' => $booking_id, 'params' => $request->all()]);
        $currentDateTime = Carbon::now()->setTimezone('Asia/Kolkata');
        $oneHourAgo = $currentDateTime->subHour();
        $user = Auth::guard('api-carhost')->user();

        $carEligibilityIds = CarEligibility::where('car_hosts_id', $user->id)->pluck('vehicle_id')->toArray();
        $rentalBooking = RentalBooking::with(['vehicle.model.manufacturer', 'vehicle.properties', 'vehicle.features', 'vehicle.images'])->whereIn('vehicle_id', $carEligibilityIds)->with([
            'vehicle' => function ($query) {
    ...
    {
        Log::info('HOST_DEBUG: getHomeVehicleStatuses called', ['params' => $request->all()]);
    ...
    {
        Log::info('HOST_DEBUG: getPickupLocations called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
        ...
        if (Auth::guard('api-carhost')->check() && $this->userAuthDetails) {
            $carPickupLocation = CarHostPickupLocation::where('car_hosts_id', $this->userAuthDetails->id)
                ->with([
                    'carHostParkingVehicleImgs' => function ($query) {
    ...
    {
        Log::info('HOST_DEBUG: getVehiclePickupLocationDetails called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
    ...
    {
        Log::info('HOST_DEBUG: getFuelAndTransmissions called', ['params' => $request->all()]);
        $typeId = $request->input('vehicle_type_id');
    ...
    {
        Log::info('HOST_DEBUG: getVehicleDetails called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
            'vehicle_id' => 'required|exists:vehicles,vehicle_id',
        ]);
        if ($validator->fails()) {
    ...
    {
        Log::info('HOST_DEBUG: getPricingControl called', ['params' => $request->all()]);
        $minPrice = 100;
        $maxPrice = 200;
        ...
        $vehicle = Vehicle::select('vehicle_id', 'model_id', 'rental_price', 'extra_km_rate', 'deposit_amount')->find($request->vehicle_id);
        if (!$vehicle) {
            return $this->errorResponse('Vehicle not found');
        }
    ...
    {
        Log::info('HOST_DEBUG: fuelTypes called');
        $typeId = $request->input('vehicle_type_id');
    ...
    {
        Log::info('HOST_DEBUG: generateStartOtp called', ['booking_id' => $booking_id]);
    ...
    {
        Log::info('HOST_DEBUG: generateEndOtp called', ['booking_id' => $booking_id]);
    ...
    {
        Log::info('HOST_DEBUG: getVehicleInfo called', ['params' => $request->all()]);
        $detailArr = [];
        $startYear = 2010;
        $currentYear = date('Y');
        $yearArr = [];
    ...
    {
        Log::info('HOST_DEBUG: getCities called');
    ...
    {
        Log::info('HOST_DEBUG: getCommonDetails called', ['params' => $request->all()]);
        $typeArr = $finalArr = [];
    ...
    {
        Log::info('HOST_DEBUG: getInsurancePucRcDetails called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
            'vehicle_id' => 'required|exists:vehicles,vehicle_id',
        ]);
        if ($validator->fails()) {
    ...
    {
        Log::info('HOST_DEBUG: getVehicleSteps called', ['params' => $request->all()]);
        $validator = Validator::make($request->all(), [
            'vehicle_id' => 'required|exists:vehicles,vehicle_id',
        ]);
        if ($validator->fails()) {
