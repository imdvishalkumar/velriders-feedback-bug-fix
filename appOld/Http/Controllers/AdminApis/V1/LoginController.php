...
class LoginController extends Controller
{
    ...
    public function adminLogin(Request $request)
    {
        ...
        logAdminActivities("Admin Logged In: " . $adminUser->username, null, null, null, $adminUser->admin_id);
    ...
    public function bookingInvoiceData(Request $request, $bookingId)
    {
        ...
        $groupedTotals = [
            ...
            12 => ['rate' => 0, 'tax' => 0, 'vehicle_commission_rate' => 0, 'vehicle_commission_tax' => 0],
            ...
            foreach ($calculationDetails as $key => $value) {
                ...
                if ($value->type == 'new_booking' && $value->paid == 1) {
                    ...
                    if (isset($groupedTotals[$gstKey])) {
                        $groupedTotals[$gstKey]['rate'] += ($value->trip_amount - $value->coupon_discount);
                    ...
                    if ($data->booking_id != 1805) {
                        ...
                        $gstKey = (int) $taxPercent;
                        if (isset($groupedTotals[$gstKey])) {
                            $groupedTotals[$gstKey]['rate'] += $additionalCharges;
                            $groupedTotals[$gstKey]['tax'] += ($value->tax_amt - $value->vehicle_commission_tax_amt);
                        }
                    } else {
                        $amountDue += 227617.59;
    ...
    public function vehicleChangeInvoiceData(Request $request, $historyId)
    {
        ...
        // Calculate the active period of THIS vehicle in history
        ...
        // Override the vehicle for rendering
        if ($history->newVehicle) {
            $data->setRelation('vehicle', $history->newVehicle);
        }
        ...
        if (is_countable($calculationDetails) && count($calculationDetails) > 0) {
            foreach ($calculationDetails as $key => $value) {
                ...
                // If end_date is missing or same as start_date for specific types, try to use booking dates
                ...
                if ($overlapEnd->greaterThan($overlapStart)) {
                    ...
                    // Completion charges belong to the final vehicle
                    if (!$nextHistory) {
        ...
        $carHost = $history->newVehicle->vehicleEligibility->carHost ?? null;

        $pdf = PDF::loadView('vehicle-change-invoice', compact(
            'history',
            'data',
            'companyDetails',
            'carHost',
        ...
        return $pdf->stream('vehicle-change-host-invoice-' . $historyId . '.pdf');
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        if ($user != '') {
            ...
            $user->save();
            logAdminActivities("Admin Password Reset for username: " . $user->username, null, null, null, $user->admin_id);
