...
class AdminBookingController extends Controller
{
        ...
        if ($page !== null && $pageSize !== null) {
            ...
            if (isset($bookingDetails) && is_countable($bookingDetails) && count($bookingDetails) > 0) {
                ...
                $bookingDetails->each(function ($booking) {
                    ...
                    $tripDurationHours = $booking->rental_duration_minutes / 60;
                    $booking->km_limit = calculateKmLimit($tripDurationHours, null, $booking->vehicle_id);
    ...
    public function getPenaltyDetails(Request $request)
    {
        ...
        if (!$booking->unlimited_kms) {
            $kilometerLimit = calculateKmLimit($tripDurationHours, null, $booking->vehicle_id);
    ...
    protected function getStartJourneyOtpStatus($bookingId)
    {
        ...
        if ($booking != '') {
            ...
            if ($currentDate >= $adjustedPickupDate && $currentDate <= $adjustedReturnDate && strtolower($booking->status) == 'confirmed' && strtolower($booking->customer->documents['dl']) == 'approved' && strtolower($booking->customer->documents['govtid']) == 'approved' && $vehicleTypeStatus) {
    ...
    public function addBooking(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'final_amt_status' => 'nullable|in:0,1',
        ...
        $calculationDetails = $rentalBooking->computeRentalCostDetails($rentalPrice, $tripDurationMinutes, $request->input('unlimited_km', false), $request->coupon_code, $startDate, $endDate, $typeId, null, 'new_booking', $customerId, $request->payment_mode, $request->ref_number, $vehicleCommissionPercent, $taxRate, $tripAmt, 0, $vehicle->vehicle_id, 1, $finalAmt, $finalAmtStatus);
    ...
    public function getAddBookingCalculation(Request $request)
    {
        ...
        if ($customerId != '' && $vehicleId != '' && $bookingStartDate != '' && $bookingEndDate != '') {
            ...
            $calculationDetails = $rentalBooking->computeRentalCostDetails($rentalPrice, $tripDurationMinutes, $unlimitedKm, $couponCode, $startDate, $endDate, $typeId, false, 'new_booking', $customerId, NULL, NULL, $vehicleCommissionPercent, $taxRate, $tripAmt, $unlimitedStatus, $vehicleId, 1, $finalAmt, $finalAmtStatus);
    ...
    public function getBookingPreviewData(Request $request)
    {
        ...
        $bookingTransaction = '';
        $rentalBookingDetails = RentalBooking::with(['customer:customer_id,firstname,lastname,dob,email,mobile_number,billing_address,shipping_address,email_verified_at,is_blocked', 'vehicle', 'payment', 'adminPenalties'])->where('booking_id', $request->booking_id)->first();
        if ($rentalBookingDetails != '') {
            $bookingTransaction = BookingTransaction::select('booking_id', 'additional_charges', 'additional_charges_info', 'late_return', 'exceeded_km_limit')->where(['booking_id' => $rentalBookingDetails->booking_id, 'type' => 'completion'])->orderBy('id', 'desc')->first();
        }

        $ownerBillingAddress = '';
        if ($rentalBookingDetails && $rentalBookingDetails->vehicle_id) {
            $checkIsHostVehicle = \App\Models\CarEligibility::where(['vehicle_id' => $rentalBookingDetails->vehicle_id, 'is_deleted' => 0])->with('carHost')->first();
            if ($checkIsHostVehicle && $checkIsHostVehicle->carHost && !empty($checkIsHostVehicle->carHost->billing_address)) {
                $ownerBillingAddress = $checkIsHostVehicle->carHost->billing_address;
            } else {
                $companyDetail = \App\Models\CompanyDetail::first();
                $ownerBillingAddress = ($companyDetail && !empty($companyDetail->address)) ? $companyDetail->address : 'Shailesh Car & Bikes Pvt. Ltd., Jamnagar';
            }
        } else {
            $companyDetail = \App\Models\CompanyDetail::first();
            $ownerBillingAddress = ($companyDetail && !empty($companyDetail->address)) ? $companyDetail->address : 'Shailesh Car & Bikes Pvt. Ltd., Jamnagar';
        ...
        if ($rentalBookingDetails) {
            $rentalBookingDetails->setAttribute('owner_billing_address', $ownerBillingAddress);
            $tripDurationHours = $rentalBookingDetails->rental_duration_minutes / 60;
            $kmLimit = calculateKmLimit($tripDurationHours, null, $rentalBookingDetails->vehicle_id);
            $rentalBookingDetails->setAttribute('km_limit', $kmLimit);
        }
        ...
        $vehicleUpdateHistory = VehicleUpdateHistory::with(['oldVehicle', 'newVehicle', 'updatedBy'])
            ...
            ->map(function ($history) {
                return [
                    ...
                    'invoice_pdf' => url('api/admin/v1/vehicle-change-invoice/' . $history->id),
                ];
            });

        $data = [
            'rentalBookingDetails' => $rentalBookingDetails,
            'owner_billing_address' => $ownerBillingAddress,
            // 'vehicles' => $vehicles,
    ...
    public function bookingPreviewActions(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'booking_id' => 'required|exists:rental_bookings,booking_id',
            'vehicle_id' => 'nullable|exists:vehicles,vehicle_id',
            'vehicle_change_reason' => 'nullable|string|max:1000',
            'vehicle_change_datetime' => 'nullable|date',
            ...
            'trip_amt' => 'nullable',
            'end_km' => 'nullable',
            'end_date_time' => 'nullable|date_format:d-m-Y H:i',
        ...
        if ($request->preview_action == 'update_vehicle') {
            ...
            \Log::info('vehicle_change_datetime request data:', $request->all());
            $oldVal = $newVal = '';
        ...
        if ($request->preview_action == 'start_journey') {
            ...
            $customerDocument = CustomerDocument::where(['customer_id' => $rentalBooking->customer_id, 'is_blocked' => 1])->exists();
            if ($customerDocument) {
        ...
        if ($request->preview_action == 'extend_price_summary') {
            ...
            $kmLimit = calculateKmLimit($tripDurationMinutes / 60, null, $rentalBooking->vehicle_id);
        ...
        if ($request->preview_action == 'end_journey') {
            ...
            // Use admin-selected end date/time if provided, otherwise fall back to now()
            if ($request->end_date_time && $request->end_date_time != '') {
                $rentalBooking->end_datetime = \Carbon\Carbon::createFromFormat('d-m-Y H:i', $request->end_date_time);
            } else {
        ...
        if ($request->preview_action == 'cancel_journey') {
            ...
            $cancelData = [
                'cancel_reason' => $request->cancel_reason ?? 'Cancelled by Administrator',
                'cancelled_by' => 'Admin'
            ];
