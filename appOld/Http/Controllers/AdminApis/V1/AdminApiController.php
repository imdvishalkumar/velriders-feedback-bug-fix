...
class AdminApiController extends Controller
{
    ...
    public function getDashboardDetails(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'pending_page' => 'nullable|numeric',
            'admin_page' => 'nullable|numeric',
            'customer_page' => 'nullable|numeric',
        ...
        ])->with('vehicle', 'cancelRentalBooking')
            ->select('booking_id', 'customer_id', 'vehicle_id', 'pickup_date', 'return_date', 'total_cost', 'rental_type', 'status', 'unlimited_kms', 'rental_duration_minutes')
        ...
        if (!empty($canceledBookingDetails->items()) && is_iterable($canceledBookingDetails->items())) {
            collect($canceledBookingDetails->items())->each(function ($item) {
                ...
                $cancelledBy = "Not Specified";

                if ($item->cancelRentalBooking) {
                    if (!empty($item->cancelRentalBooking->data_json)) {
                        $cancelData = json_decode($item->cancelRentalBooking->data_json, true);
                        if (isset($cancelData['cancel_reason'])) {
                            $cancelReason = $cancelData['cancel_reason'];
                        }
                        if (isset($cancelData['cancelled_by'])) {
                ...
                $item->cancelled_by = $cancelledBy;
                $tripDurationHours = $item->rental_duration_minutes / 60;
                $item->km_limit = calculateKmLimit($tripDurationHours);
            });
        }
        ...
        if (!empty($runningBookingDetails->items()) && is_iterable($runningBookingDetails->items())) {
            collect($runningBookingDetails->items())->each(function ($item) {
                $tripDurationHours = $item->rental_duration_minutes / 60;
                $item->km_limit = calculateKmLimit($tripDurationHours);
            });
        }
        ...
        $pendingPaymentCountQuery = AdminRentalBooking::whereHas('bookingTransactions', function ($q) {
            $q->where('paid', 0)->where('amount_to_pay', '>', 0);
        })
            ->whereDate('created_at', '>=', $startDate)
            ->whereDate('created_at', '<=', $endDate);

        $applyBookingFilters($pendingPaymentCountQuery);
        $pendingPaymentCount = $pendingPaymentCountQuery->count();

        $pendingPaymentDetailsQuery = AdminRentalBooking::with([
            'customer' => function ($query) {
                $query->select('customer_id', 'country_code', 'mobile_number', 'email', 'firstname', 'lastname', 'dob', 'profile_picture_url');
            },
            'vehicle' => function ($query) {
                $query->select('vehicle_id', 'model_id', 'year', 'description', 'color', 'license_plate', 'availability')->with('model');
            },
            'bookingTransactions' => function ($q) {
                $q->where('paid', 0)->where('amount_to_pay', '>', 0);
            }
        ])->select('booking_id', 'customer_id', 'vehicle_id', 'pickup_date', 'return_date', 'total_cost', 'rental_type', 'status', 'created_at', 'unlimited_kms', 'rental_duration_minutes')
            ->whereHas('bookingTransactions', function ($q) {
                $q->where('paid', 0)->where('amount_to_pay', '>', 0);
            })
            ->whereDate('created_at', '>=', $startDate)
            ->whereDate('created_at', '<=', $endDate);

        $applyBookingFilters($pendingPaymentDetailsQuery);
        $pendingPaymentDetails = $pendingPaymentDetailsQuery->paginate($perPage, ['*'], 'pending_page');

        if (!empty($pendingPaymentDetails->items()) && is_iterable($pendingPaymentDetails->items())) {
            collect($pendingPaymentDetails->items())->each(function ($item) {
                $item->pending_amount = $item->bookingTransactions->sum('amount_to_pay');
                if ($item->vehicle) {
                    $item->vehicle->makeHidden(['branch_id', 'rental_price', 'extra_km_rate', 'extra_hour_rate', 'availability_calendar', 'commission_percent', 'publish', 'chassis_no']);
                }
                $tripDurationHours = $item->rental_duration_minutes / 60;
        ...
        $adminBookingCountQuery = AdminRentalBooking::whereHas('payment', function ($q) {
            $q->where('razorpay_order_id', 'admin_booking');
        })
            ->whereDate('created_at', '>=', $startDate)
            ->whereDate('created_at', '<=', $endDate);

        $applyBookingFilters($adminBookingCountQuery);
        $adminBookingCount = $adminBookingCountQuery->count();

        $adminBookingDetailsQuery = AdminRentalBooking::with([
            'customer' => function ($query) {
                $query->select('customer_id', 'country_code', 'mobile_number', 'email', 'firstname', 'lastname', 'dob', 'profile_picture_url');
            },
            'vehicle' => function ($query) {
        ...
        $applyBookingFilters($adminBookingDetailsQuery);
        $adminBookingDetails = $adminBookingDetailsQuery->paginate($perPage, ['*'], 'admin_page');

        if (!empty($adminBookingDetails->items()) && is_iterable($adminBookingDetails->items())) {
            collect($adminBookingDetails->items())->each(function ($item) {
                if ($item->vehicle) {
                    $item->vehicle->makeHidden(['branch_id', 'rental_price', 'extra_km_rate', 'extra_hour_rate', 'availability_calendar', 'commission_percent', 'publish', 'chassis_no']);
                }
                $tripDurationHours = $item->rental_duration_minutes / 60;
        ...
        $customerBookingCountQuery = AdminRentalBooking::where(function ($query) {
        ...
        $applyBookingFilters($customerBookingCountQuery);
        $customerBookingCount = $customerBookingCountQuery->count();

        $customerBookingDetailsQuery = AdminRentalBooking::with([
            'customer' => function ($query) {
                $query->select('customer_id', 'country_code', 'mobile_number', 'email', 'firstname', 'lastname', 'dob', 'profile_picture_url');
            },
            'vehicle' => function ($query) {
        ...
        $applyBookingFilters($customerBookingDetailsQuery);
        $customerBookingDetails = $customerBookingDetailsQuery->paginate($perPage, ['*'], 'customer_page');

        if (!empty($customerBookingDetails->items()) && is_iterable($customerBookingDetails->items())) {
            collect($customerBookingDetails->items())->each(function ($item) {
                if ($item->vehicle) {
                    $item->vehicle->makeHidden(['branch_id', 'rental_price', 'extra_km_rate', 'extra_hour_rate', 'availability_calendar', 'commission_percent', 'publish', 'chassis_no']);
                }
                $tripDurationHours = $item->rental_duration_minutes / 60;
    ...
    public function updateSettingsDetails(Request $request)
    {
        ...
        if ($updateFlag == 'app_detail') {
            ...
            $setting->save();
            logAdminActivities("Vehicle Show Flag Updated to: " . ($request->show_all_vehicle == 1 ? 'On' : 'Off'));
            ...
            if ($request->booking_gap != '' && $request->booking_gap != 0) {
                ...
                logAdminActivities("Vehicle Booking Gap Updated to: " . $request->booking_gap . " minutes");
            } else {
            ...
            logAdminActivities("Vehicle Offer Dates Updated");
            ...
            $setting->save();
            logAdminActivities("Payment Gateway Details Updated");
            ...
            if ($request->reward_type != '' && $request->reward_val != '') {
                ...
                logAdminActivities("Refer & Earn Details Updated");
            } else {
            ...
            logAdminActivities("Customer Document Verify Limit Updated to: " . $request->cust_doc_verif_limits);
            return $this->successResponse($setting, $message);
            ...
            logAdminActivities("Host Location Km. Distance Value Updated to: " . $request->location_km_distance_val);
            return $this->successResponse($setting, $message);
        }
    }
    ...
    {
        logAdminActivities("Admin Logged Out");
