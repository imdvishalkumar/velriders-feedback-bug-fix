...
class CarHostManagementController extends Controller
{
    public function getCarHosts(Request $request)
    {
        ...
        $carHosts = CarHost::select('id', 'country_code', 'mobile_number', 'email', 'firstname', 'lastname', 'pan_number', 'dob', 'profile_picture_url', 'created_at', 'is_blocked', 'gst_number', 'business_name')
            ->where('is_deleted', 0)
    ...
    public function getCarHostChanges(Request $request)
    {
        ...
        $sectionId = $request->input('section_id') ?? $request->input('status');
        $data = [];

        $shouldFetch = function ($keys) use ($sectionId) {
            if (!$sectionId)
                return true;
            return in_array($sectionId, (array) $keys);
        };

        if ($shouldFetch(['pickup_locations', 'tab-3', 'location-list'])) {
            ...
            if ($search) {
                $carHostPickupLocationTemp->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ...
                        ->orWhere('car_host_pickup_location_temps.name', 'LIKE', "%$search%")
                        ->orWhere('car_host_pickup_location_temps.location', 'LIKE', "%$search%")
                        ->orWhereRaw('CONCAT(h.firstname, " ", h.lastname) LIKE ?', ["%{$search}%"]);
                });
            }
        }

        if ($shouldFetch(['features', 'tab-1', 'features-list'])) {
            $carHostVehicleFeatureTemp = DB::table('car_host_vehicle_feature_temps as f')
                ...
                ->join('vehicle_manufacturers as m', 'm.manufacturer_id', '=', 'vm.manufacturer_id')
                ->select(
                    'f.vehicles_id',
            ...
            if ($search) {
                $carHostVehicleFeatureTemp->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ->orWhere('h.lastname', 'LIKE', "%$search%")
                        ->orWhere('h.email', 'LIKE', "%$search%")
                        ->orWhere('h.mobile_number', 'LIKE', "%$search%")
                        ...
                        ->orWhereRaw('CONCAT(m.name, " ", vm.name) LIKE ?', ["%{$search}%"])
                        ->orWhereRaw('CONCAT(h.firstname, " ", h.lastname) LIKE ?', ["%{$search}%"]);
                });
            }
        }

        if ($shouldFetch(['images', 'tab-2', 'images-list'])) {
            ...
            if ($search) {
                $carHostVehiclesImageTemp->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ->orWhere('h.lastname', 'LIKE', "%$search%")
                        ->orWhere('h.email', 'LIKE', "%$search%")
                        ->orWhere('h.mobile_number', 'LIKE', "%$search%")
        ...
        if ($shouldFetch(['vehicle_details', 'tab-4', 'vehicle-details-list'])) {
            $carHostVehicleDetails = Vehicle::join('car_eligibilities as v', 'v.vehicle_id', '=', 'vehicles.vehicle_id')
                ->join('car_hosts as h', 'h.id', '=', 'v.car_hosts_id')
            ...
            if ($search) {
                $carHostVehicleDetails->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ->orWhere('h.lastname', 'LIKE', "%$search%")
                        ->orWhere('h.email', 'LIKE', "%$search%")
                        ->orWhere('h.mobile_number', 'LIKE', "%$search%")
        ...
        if ($shouldFetch(['carHostVehicleDocuments', 'tab-5', 'host-vehicle-documents-list'])) {
            ...
            if ($search) {
                $carHostVehicleDocuments->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ->orWhere('h.lastname', 'LIKE', "%$search%")
                        ->orWhere('h.email', 'LIKE', "%$search%")
                        ->orWhere('h.mobile_number', 'LIKE', "%$search%")
        ...
        if ($shouldFetch(['Price_details', 'tab-6', 'vehicle-price-details-list'])) {
            $VehiclePriceDetailTemp = VehiclePriceDetailTemp::join('car_eligibilities as v', 'v.vehicle_id', '=', 'vehicle_price_detail_temps.vehicle_id')
                ->join('car_hosts as h', 'h.id', '=', 'v.car_hosts_id')
                ...
                ->whereIn('vehicle_price_detail_temps.id', function ($q) {
                    $q->select(DB::raw('MAX(id)'))
                        ->from('vehicle_price_detail_temps')
                        ->groupBy('vehicle_id');
                });
            if ($search) {
                $VehiclePriceDetailTemp->where(function ($q) use ($search) {
                    $q->where('h.firstname', 'LIKE', "%$search%")
                        ->orWhere('h.lastname', 'LIKE', "%$search%")
                        ->orWhere('h.email', 'LIKE', "%$search%")
                        ->orWhere('h.mobile_number', 'LIKE', "%$search%")
        ...
        if ($page !== null && $pageSize !== null) {
            if (isset($carHostPickupLocationTemp)) {
            ...
            if (isset($carHostVehicleFeatureTemp)) {
            ...
            if (isset($carHostVehiclesImageTemp)) {
            ...
            if (isset($carHostVehicleDetails)) {
            ...
            if (isset($carHostVehicleDocuments)) {
            ...
            if (isset($VehiclePriceDetailTemp)) {
                $VehiclePriceDetailTemp = $VehiclePriceDetailTemp->orderBy('created_at', 'desc')->paginate($pageSize, ['*'], 'page', $page);
                ...
                $VehiclePriceDetailTempArray = json_decode(json_encode($VehiclePriceDetailTemp->getCollection()->values()), FALSE);
                $VehiclePriceDetailTemp = [
                    'carHostVehiclePriceDetails' => $VehiclePriceDetailTempArray,
                    'pagination' => [
                        'total' => $VehiclePriceDetailTemp->total(),
                        'per_page' => $VehiclePriceDetailTemp->perPage(),
                        'current_page' => $VehiclePriceDetailTemp->currentPage(),
                        'last_page' => $VehiclePriceDetailTemp->lastPage(),
                        'from' => ($VehiclePriceDetailTemp->currentPage() - 1) * $VehiclePriceDetailTemp->perPage() + 1,
                        'to' => min($VehiclePriceDetailTemp->currentPage() * $VehiclePriceDetailTemp->perPage(), $VehiclePriceDetailTemp->total()),
                    ]
                ];
            }
        } else {
            ...
            if (isset($carHostVehicleFeatureTemp)) {
                ...
                if (isset($carHostVehicleFeatureTemp) && $carHostVehicleFeatureTemp->isNotEmpty()) {
                    foreach ($carHostVehicleFeatureTemp as $k => $v) {
                        $vehicle = Vehicle::where('vehicle_id', $v->vehicles_id)->first();
                        if ($vehicle) {
                            $vehicle->makeHidden(['branch_id', 'year', 'description', 'availability', 'is_deleted', 'extra_km_rate', 'extra_hour_rate', 'availability_calendar', 'commission_percent', 'chassis_no', 'created_at', 'updated_at', 'nick_name', 'banner_image', 'banner_images', 'regular_images', 'host_banner_images', 'host_regular_images', 'rating', 'total_rating', 'trip_count', 'model']);
        ...
        return $this->successResponse($data, 'Host changes data get successfully');
    }
    ...
    public function getUnpublishVehices(Request $request)
    {
        ...
        $vehicle = Vehicle::select('vehicles.vehicle_id', 'vehicles.model_id', 'vehicles.year', 'vehicles.description', 'vehicles.color', 'vehicles.license_plate', 'vehicles.rental_price', 'vehicles.apply_for_publish', 'vehicles.publish')
            ->join('vehicle_models as vm', 'vm.model_id', '=', 'vehicles.model_id')
            ->join('vehicle_manufacturers as m', 'm.manufacturer_id', '=', 'vm.manufacturer_id')
        ...
        if ($search) {
            $vehicle = $vehicle->where(function ($query) use ($search) {
                $query->whereRaw('LOWER(vehicles.license_plate) LIKE LOWER(?)', ["%$search%"])
                    ->orWhereRaw('LOWER(vm.name) LIKE LOWER(?)', ["%$search%"])
                    ->orWhereRaw('LOWER(m.name) LIKE LOWER(?)', ["%$search%"])
                    ->orWhereRaw('CONCAT(m.name, " ", vm.name) LIKE ?', ["%{$search}%"])
                    ->orWhereExists(function ($q) use ($search) {
                        $q->select(DB::raw(1))
                            ->from('car_eligibilities as ce')
                            ->join('car_hosts as ch', 'ce.car_hosts_id', '=', 'ch.id')
                            ->whereColumn('ce.vehicle_id', 'vehicles.vehicle_id')
                            ->where(function ($sub) use ($search) {
                                $sub->whereRaw('LOWER(ch.firstname) LIKE LOWER(?)', ["%$search%"])
                                    ->orWhereRaw('LOWER(ch.lastname) LIKE LOWER(?)', ["%$search%"])
                                    ->orWhereRaw('CONCAT(ch.firstname, " ", ch.lastname) LIKE ?', ["%{$search}%"])
                                    ->orWhereRaw('LOWER(ch.email) LIKE LOWER(?)', ["%$search%"])
                                    ->orWhereRaw('LOWER(ch.mobile_number) LIKE LOWER(?)', ["%$search%"]);
                            });
                    });
            });
        }
    ...
    public function deleteHostBank(Request $request)
    {
        ...
        if ($carHostBank != '') {
            ...
            if ($checkPrimaryBank == '') {
                ...
                $carHostBank->save();
                logAdminActivities("Host Bank Deleted", $carHostBank);
    ...
    public function storeHostUpdatedFeatures(Request $request)
    {
        ...
        if (isset($carHostVehicleFeatureTemp) && is_countable($carHostVehicleFeatureTemp) && count($carHostVehicleFeatureTemp) > 0) {
            ...
            if (is_countable($carHostVehicleFeatureTemp) && count($carHostVehicleFeatureTemp) > 0) {
                ...
                $this->publishVehicleById($request->vehicle_id);
                logAdminActivities("Host-Vehicle Feature Approved for Vehicle ID: " . $request->vehicle_id);
    ...
    public function rejectHostUpdatedFeatures(Request $request)
    {
        ...
        logAdminActivities("Host-Vehicle Feature Rejected for Vehicle ID: " . $request->vehicle_id);
    ...
    public function storeHostUpdatedImages(Request $request)
    {
        ...
        $this->publishVehicleById($request->vehicle_id);
        logAdminActivities("Host-Vehicle Images Approved for Vehicle ID: " . $request->vehicle_id);
    ...
    public function rejectHostUpdatedImages(Request $request)
    {
        ...
        $tempImages = CarHostVehicleImageTemp::where('vehicles_id', $request->vehicle_id)->get();
        foreach ($tempImages as $v) {
        ...
        logAdminActivities("Host-Vehicle Images Rejected for Vehicle ID: " . $request->vehicle_id);
    ...
    public function storeHostUpdatedLocation(Request $request)
    {
        ...
        if (isset($carHostPickupLocationTemp) && $carHostPickupLocationTemp != '') {
            ...
            logAdminActivities("Host-Vehicle Location Approved for Pickup Location ID: " . $request->host_pickup_location_id);
    ...
    public function storeHostUpdatedVehicleDetails(Request $request)
    {
        ...
        if (isset($vehicle) && $vehicle != '') {
            ...
            $this->publishVehicleById($request->vehicle_id);
            logAdminActivities("Host-Vehicle Details Approved for Vehicle ID: " . $request->vehicle_id);
    ...
    public function rejectHostUpdatedVehicleDetails(Request $request)
    {
        ...
        if (isset($vehicle) && $vehicle != '') {
            ...
            $vehicle->updated_year = NULL;
            $vehicle->save();
            logAdminActivities("Host-Vehicle Details Rejected for Vehicle ID: " . $request->vehicle_id);
            return $this->successResponse(null, 'New Vehicle details are Rejected sucessfully');
        } else {
            return $this->errorResponse('Vehicle details not found');
        }
    }
    ...
    public function storeHostUpdatedVehicleDocuments(Request $request)
    {
        ...
        if (isset($vehicleDocumentTemp) && is_countable($vehicleDocumentTemp) && count($vehicleDocumentTemp) > 0) {
            ...
            if (is_countable($vehicleDocumentTemp) && count($vehicleDocumentTemp)) {
                ...
                $this->publishVehicleById($request->vehicle_id);
                logAdminActivities("Host-Vehicle Documents Approved for Vehicle ID: " . $request->vehicle_id . " - Type: " . $request->document_type);
    ...
    public function rejectHostUpdatedVehicleDocuments(Request $request)
    {
        ...
        $tempDocs = VehicleDocumentTemp::where(['vehicle_id' => $request->vehicle_id])->where('document_type', $request->document_type)->get();
        foreach ($tempDocs as $v) {
            if (isset($v->document_image_url)) {
                $filePath = public_path() . '/images/documents/' . basename($v->document_image_url);
                if (file_exists($filePath)) {
                    unlink($filePath);
        ...
        logAdminActivities("Host-Vehicle Documents Rejected for Vehicle ID: " . $request->vehicle_id . " - Type: " . $request->document_type);
    ...
    public function storeHostUpdatedVehiclePrices(Request $request)
    {
        ...
        if (isset($vehiclePriceDetailTemp) && is_countable($vehiclePriceDetailTemp) && count($vehiclePriceDetailTemp) > 0) {
            ...
            if (is_countable($vehiclePriceDetailTemp) && count($vehiclePriceDetailTemp)) {
                ...
                if ($checkVehicle != '' && (isset($checkVehicle->updated_extra_km_rate) && ($checkVehicle->updated_extra_km_rate != NULL || $checkVehicle->updated_extra_km_rate != ''))) {
                    ...
                    // Sync base rental_price if not set
                    if ($checkVehicle->rental_price <= 0) {
                        $firstTemp = $vehiclePriceDetailTemp->first();
                        if ($firstTemp) {
                            $checkVehicle->rental_price = $firstTemp->rental_price;
                        }
                    }
                    $checkVehicle->save();
                } else if ($checkVehicle != '') {
                    // Even if no extra km rate updates, still sync rental_price if missing
                    if ($checkVehicle->rental_price <= 0) {
                        $firstTemp = $vehiclePriceDetailTemp->first();
                        if ($firstTemp) {
                            $checkVehicle->rental_price = $firstTemp->rental_price;
                ...
                $this->publishVehicleById($request->vehicle_id);
                logAdminActivities("Host-Vehicle Prices Approved for Vehicle ID: " . $request->vehicle_id);
    ...
    public function rejectHostUpdatedVehiclePrices(Request $request)
    {
        ...
        logAdminActivities("Host-Vehicle Prices Rejected for Vehicle ID: " . $request->vehicle_id);
        return $this->successResponse(null, 'New vehicle prices are Rejected sucessfully');
    }

    private function publishVehicleById($vehicleId)
    {
        $vehicle = Vehicle::find($vehicleId);
        if ($vehicle != '') {
            if ($vehicle->rental_price <= 0) {
                $priceDetail = VehiclePriceDetail::where('vehicle_id', $vehicleId)->first();
                if ($priceDetail) {
                    $vehicle->rental_price = $priceDetail->rental_price;
                    $vehicle->save();
                }
            }
    ...
    public function deleteHostVehicleImage(Request $request)
    {
        ...
        if ($carHostVehicleImage != '') {
            ...
            $carHostVehicleImage->delete();
            logAdminActivities("Host Vehicle Image Deleted", $carHostVehicleImage);
