...
{   
  ...
  // protected $setting;
    ...
    public function getPriceDetails(Request $request)
    {
        ...
        // Get user's test status - default to 0 (normal user) if not authenticated or not a test user
        $isTestUser = 0;
        if (Auth::guard('api')->check() && isset($this->userAuthDetails->is_test_user)) {
            $isTestUser = $this->userAuthDetails->is_test_user ?? 0;
        } elseif ($user && isset($user->is_test_user)) {
            $isTestUser = $user->is_test_user ?? 0;
        }
        ...
        $checkedBookedVehicele = checkedBookedVehicele($vehicleId, $startDate, $endDate, $bookingGap, NULL, $isTestUser);
    ...
    public function store(Request $request)
    {
        ...
        //CHECKED IF THIS VEHICLE ID BOOKED OR NOT WITH OTHER ORDER
        $checkedBookedVehicele = checkedBookedVehicele($vehicle->vehicle_id, $startDate, $endDate, $bookingGap, NULL, $IsTestUser);
        if($checkedBookedVehicele != ''){
            return $this->errorResponse($checkedBookedVehicele);
        }
        //CHECKED IF USER HAS BOOKED ANOTHER VEHICLE ON SAME TIME OR NOT
        ...
        if($IsTestUser == 1){
            $rentalBooking->is_test_booking = '1';
        }
