...
class CarHostVehicleDetailsController extends Controller
{
    ...
    public function getBookingDetails(Request $request, $booking_id)
    {
        ...
        if ($rentalBooking != '') {
            ...
            $hiddenFields = ['from_branch_id', 'to_branch_id', 'customer_id', 'rental_duration_minutes', 'penalty_details', 'start_otp', 'end_otp', 'unlimited_kms', 'button_visiblity', 'total_cost', 'amount_paid', 'rental_type', /*'calculation_details', */ 'data_json', 'start_datetime', 'end_datetime', 'sequence_no', 'created_at', 'updated_at', 'pay_now_status', 'admin_penalty_amount'];
            $showCustomer = ($rentalBooking->status != 'pending') || (isset($rentalBooking->pickup_date) && (Carbon::parse($rentalBooking->pickup_date)->timestamp - $currentDateTime->timestamp <= 3600));
            if (!$showCustomer) {
                $hiddenFields[] = 'customer';
                $hiddenFields[] = 'dl_status';
                $hiddenFields[] = 'govtid_status';
                $hiddenFields[] = 'invoice_pdf';
                $hiddenFields[] = 'summary_pdf';
                $hiddenFields[] = 'admin_invoice_pdf';
                $hiddenFields[] = 'admin_summary_pdf';
                $hiddenFields[] = 'admin_customer_aggrement';
            }
            $rentalBooking->setHidden($hiddenFields);
