            ...
            if ($bookingId != '') {
                if ($startDate != '' && $endDate != '') {
                    if ($setting != '' && $setting->show_all_vehicle != 1) {
                        $vehicles = $vehicles->whereDoesntHave('rentalBookings', function ($query) use ($startDate, $endDate) {
                            $query->whereIn('status', ['running', 'confirmed'])->where(function ($query) use ($startDate, $endDate) {
