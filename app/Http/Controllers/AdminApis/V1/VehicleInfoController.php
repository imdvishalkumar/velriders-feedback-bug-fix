...
class VehicleInfoController extends Controller
{
    ...
    //                 }
    //             ])
    //             ->leftJoin('car_eligibilities', function ($join) {
    //                     $join->on('car_eligibilities.vehicle_id', '=', 'vehicles.vehicle_id')
    //                         ->where('car_eligibilities.is_deleted', '=', 0);
    //                 })
    ...
    //     }
    //     // Fetch booking dates if bookingId is provided
    //     $startDate = '';
    //     $endDate = '';
    ...
    //     if(isset($request->is_avaliable) && $request->is_avaliable == 1){
    //         if ($bookingId != '') {
    ...
    public function getVehicles(Request $request)
    {
        ...
        $vehicles = Vehicle::select(
            ...
            ->with([
                ...
                'features',
                'vehicleDocuments',
                'CarHostVehicleFeatures',
                'pricingDetails' => function ($q) {
                    $q->select('id', 'vehicle_id', 'rental_price', 'hours', 'rate', 'is_show');
                }
            ])
            ->leftJoin('car_eligibilities', function ($join) {
            ...
            ->leftJoin('vehicle_categories', 'vehicle_categories.category_id', '=', 'vehicle_models.category_id')
            ->leftJoin('branches', 'branches.branch_id', '=', 'vehicles.branch_id')
            ->leftJoin('cities as temp_cities', 'temp_cities.id', '=', 'vehicles.temp_city_id')
            ->leftJoin('cities as branch_cities', 'branch_cities.id', '=', 'branches.city_id')
            ->where('vehicles.is_deleted', 0);
        ...
        if (isset($search) && $search != '') {
            $vehicles = $vehicles->where(function ($query) use ($search) {
                $query->orWhere('vehicles.vehicle_id', $search)
                    ...
                    ->orWhereRaw("LOWER(CONCAT(SUBSTRING_INDEX(vehicle_manufacturers.name, ' ', 1), ' ', vehicle_models.name)) LIKE LOWER(?)", ["%$search%"])
                    ->orWhereRaw('LOWER(vehicles.rental_price) LIKE LOWER(?)', ["%$search%"])
                    ->orWhereRaw('LOWER(COALESCE(temp_cities.name, branch_cities.name)) LIKE LOWER(?)', ["%$search%"]);
            });
        }
        ...
        if (isset($request->is_avaliable) && $request->is_avaliable == 1) {
            if ($bookingId != '') {
                if ($startDate != '' && $endDate != '') {
                    $vehicles = $vehicles->get();
                    $vehicles = $vehicles->filter(function ($item) use ($startDate, $endDate) {
        ...
        if ($page !== null && $pageSize !== null) {
            if (!isset($request->is_avaliable) || $request->is_avaliable != 1) {
                ...
                $vehicles = $vehicles->forPage($page, $pageSize);
        ...
        if ($request->vehicle_step == 'vehicle_details') {
            ...
            if (isset($request->car_host) && $request->car_host != '') {
                ...
                if ($carHostEligibility) {
                    ...
                    $carHostEligibility->is_deleted = 0;
                    $carHostEligibility->save();
                }
                ...
                $carHostEligibility = CarEligibility::where('vehicle_id', $vehicle->vehicle_id)->first();
                if ($carHostEligibility) {
                    $carHostEligibility->is_deleted = 1;
                    $carHostEligibility->save();
                }
            }
            $oldArr = [
