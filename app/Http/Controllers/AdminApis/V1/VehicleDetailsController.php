...
class VehicleDetailsController extends Controller
{
    ...
    public function createOrUpdateVehicleTypes(Request $request)
    {
        ...
        if (empty($request->type_id)) {
            logAdminActivities("Vehicle Type Creation", null, $type);
        } else {
            $differences = compareArray($oldType, $type);
            if (count($differences) > 0) {
                logAdminActivities('Vehicle Type Updation', $oldType, $type);
            }
        }
    ...
    public function createOrUpdateVehicleCategories(Request $request)
    {
        ...
        if (empty($request->category_id)) {
            logAdminActivities("Vehicle Category Creation", null, $vehicleCategory);
        } else {
            $differences = compareArray($oldCat, $vehicleCategory);
            if (count($differences) > 0) {
                logAdminActivities('Vehicle Category Updation', $oldCat, $vehicleCategory);
            }
        }
    ...
    public function createOrUpdateFuelType(Request $request)
    {
        ...
        if (empty($request->fuel_type_id)) {
            logAdminActivities("Fuel Type Creation", null, $type);
        } else {
            $differences = compareArray($oldType, $type);
            if (count($differences) > 0) {
                logAdminActivities('Fuel Type Updation', $oldType, $type);
            }
        }
    ...
    public function createOrUpdateVehicleTrasmission(Request $request)
    {
        ...
        if ($request->transmission_id != NULL) {
            ...
            $oldTransmission = clone $transmission;
        } else {
        ...
        if (empty($request->transmission_id)) {
            logAdminActivities("Transmission Creation", null, $transmission);
        } else {
            $differences = compareArray($oldTransmission, $transmission);
            if (count($differences) > 0) {
                logAdminActivities('Transmission Updation', $oldTransmission, $transmission);
            }
        }
    ...
    public function createOrUpdateVehicleFeature(Request $request)
    {
        ...
        if ($request->feature_id != '') {
            ...
            $oldFeature = clone $vehicleFeature;
        } else {
        ...
        if (empty($request->feature_id)) {
            logAdminActivities("Vehicle Feature Creation", null, $vehicleFeature);
        } else {
            $differences = compareArray($oldFeature, $vehicleFeature);
            if (count($differences) > 0) {
                logAdminActivities('Vehicle Feature Updation', $oldFeature, $vehicleFeature);
            }
        }
    ...
    public function createOrUpdateVehicleManufacturer(Request $request)
    {
        ...
        if ($request->manufacturer_id != '') {
            ...
            $oldManufacturer = clone $vehicleManufacturer;
        } else {
        ...
        if (empty($request->manufacturer_id)) {
            logAdminActivities("Vehicle Manufacturer Creation", null, $vehicleManufacturer);
        } else {
            $differences = compareArray($oldManufacturer, $vehicleManufacturer);
            if (count($differences) > 0) {
                logAdminActivities('Vehicle Manufacturer Updation', $oldManufacturer, $vehicleManufacturer);
            }
        }
    ...
    public function createOrUpdateVehicleModel(Request $request)
    {
        ...
        if (empty($request->model_id)) {
            logAdminActivities("Vehicle Model Creation", null, $model);
        } else {
        ...
        if ($minPriceCalc != '') {
            ...
            if (is_countable($minMultipliers) && count($minMultipliers) > 0) {
                foreach ($minMultipliers as $key => $value) {
                    ...
                    $vehicleTypeName = $model->category->vehicleType->name ?? null;
                    $vehicleModelPriceDetail->trip_amount_km_limit = calculateKmLimit($key, $vehicleTypeName) . " Km";
