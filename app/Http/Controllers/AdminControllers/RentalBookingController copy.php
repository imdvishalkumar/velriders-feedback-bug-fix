        ...
        if(is_countable($calculationDetails) && count($calculationDetails) > 0){
            foreach ($calculationDetails as $key => $value) {
                ...
                if($value->type == 'new_booking' && $value->paid == 1){
                    ...
                    // REVERSE CALCULATION FROM ACTUAL PAID AMOUNT
                    // displayedAmount = total_amount - convenience_fee - vehicle_commission - vehicle_commission_tax
                    $displayedAmount = ($value->total_amount ?? 0) - ($value->convenience_fee ?? 0);
                    $displayedAmount = $displayedAmount - ($value->vehicle_commission_amount ?? 0) - ($value->vehicle_commission_tax_amt ?? 0);
                    
                    // Discount from DB
                    $discount = $value->coupon_discount ?? 0;
                    
                    // Reverse calculate: Amount = (Rate - Discount) × (1 + GST_rate)
                    // So: (Rate - Discount) = Amount / (1 + GST_rate)
                    // And: Rate = (Amount / (1 + GST_rate)) + Discount
                    $taxableAmount = $displayedAmount / (1 + $taxRateDecimal);
                    $baseRate = $taxableAmount + $discount;
                    $taxAmount = $displayedAmount - $taxableAmount;
                    
                    $newBooking['trip_amount'] = number_format($baseRate, 2);
                    $newBooking['tax_percent'] = number_format($taxPercent, 2);
                    $newBooking['tax_amount'] = number_format($taxAmount, 2);
                    ...
                    $rateTotal += $baseRate - $discount; // taxable amount
                    
                    // Group totals by GST percentage
                    $gstKey = (int)$taxPercent;
                    if(isset($groupedTotals[$gstKey])){
                        $groupedTotals[$gstKey]['rate'] += $taxableAmount;
                        $groupedTotals[$gstKey]['tax'] += $taxAmount;
                        if($discount > 0){
                            $groupedTotals[$gstKey]['discount'] += $discount;
                        }
                    }
                    ...
                    $extension['trip_amount'][] = number_format($baseRate, 2);
                    $extension['tax_percent'][] = number_format($taxPercent, 2);
                    $extension['tax_amount'][] = number_format($taxAmount, 2);
                    ...
                    // Group totals by GST percentage for completion
                    if($data->booking_id != 1805){
                        $gstKey = (int)$taxPercent;
                        if($gstKey == 0) $gstKey = 5; // Default to 5% if not determined
                        if(isset($groupedTotals[$gstKey])){
