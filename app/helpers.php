...
function calculateKmLimit($tripDurationHours, $vehicleType = null)
{
...
function checkedBookedVehicele($vehicleId, $startDate, $endDate, $bookingGap, $bookingId = NULL)
{
...
// Improved function with temporary hold check 22/01/2026
function checkVehicleStatus($vehicleId, $bookingId, $startDate, $endDate)
{
    ...
    /*
    |--------------------------------------------------------------------------
    | TEMPORARY HOLD CHECK (vehicles.availability_calendar)
    |--------------------------------------------------------------------------
    */
    $vehicle = Vehicle::select('availability_calendar')
        ->where('vehicle_id', $vehicleId)
        ->first();

    if ($vehicle && !empty($vehicle->availability_calendar)) {

        $calendar = is_string($vehicle->availability_calendar)
            ? json_decode($vehicle->availability_calendar, true)
            : $vehicle->availability_calendar;

        foreach ($calendar as $hold) {

            if (empty($hold['start_date']) || empty($hold['end_date'])) {
                continue;
            }

            $holdStart = normalizeAndParseDate($hold['start_date']);
            $holdEnd = normalizeAndParseDate($hold['end_date']);

            if (!$holdStart || !$holdEnd) {
                \Log::error('HOLD DATE STILL INVALID AFTER CLEAN', [
                    'start_raw' => $hold['start_date'],
                    'end_raw' => $hold['end_date']
                ]);
                continue;
            }
            ...
            if ($requestStartParsed < $holdEnd && $requestEndParsed > $holdStart) {
                ...
                return sprintf(
                    ...
                    $holdStart->format('d M Y, h:i A'),
                    $holdEnd->format('d M Y, h:i A'),
                    !empty($hold['reason']) ? ' (' . $hold['reason'] . ')' : ''
                );
            }
        }
    ...
    |--------------------------------------------------------------------------
    */
    $existingBooking = RentalBooking::where('vehicle_id', $vehicleId)
        ->whereIn('status', ['running', 'confirmed'])
        ->where('booking_id', '!=', $bookingId)
        ->where(function ($query) use ($requestStart, $requestEnd) {
            $query->where('pickup_date', '<=', $requestEnd)
                ->where('return_date', '>=', $requestStart);
        })
        ->exists();

    if ($existingBooking) {
        return 'The vehicle is already booked for the selected time period.';
    }

    return '';
}

function normalizeAndParseDate($dateString)
{
    try {
        ...
        // Case: 20:00 PM → remove PM
        if ($ampm && $hour >= 13) {
            $ampm = null;
        }

        // Case: 00:08 AM → convert to 12
        if ($ampm && $hour == 0) {
            $hour = 12;
        }
        ...
        if ($ampm) {
            return \Carbon\Carbon::createFromFormat(
                'd-m-Y h:i A',
                "$datePart " . sprintf('%02d:%02d', $hour, $minute) . " $ampm"
            );
        }

        return \Carbon\Carbon::createFromFormat(
            'd-m-Y H:i',
            "$datePart " . sprintf('%02d:%02d', $hour, $minute)
        );

    } catch (\Exception $e) {
        \Log::error('DATE NORMALIZE FAILED', [
            'input' => $dateString,
            'error' => $e->getMessage()
        ]);
        return null;
...
function getStartOtpText($bookingId)
{
    ...
    if ($booking != '') {
        ...
        // }
        elseif ($booking->customer->is_blocked == 1) {
            $startOtpText = "Customer is blocked";
...
function getOtpText($bookingId)
{
    ...
    // }
    elseif (strtolower($booking->status) == 'confirmed' && $booking->customer->is_blocked == 1) {
        $otpText = "Customer is blocked";
