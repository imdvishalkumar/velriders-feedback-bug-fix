...
class RentalBookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Start database transaction to prevent double booking race conditions
        DB::beginTransaction();

        try {
            // Retrieve vehicle details with lockForUpdate to block concurrent requests for the same vehicle
            $vehicle = Vehicle::select('vehicle_id', 'model_id', 'availability', 'rental_price', 'availability_calendar', 'commission_percent', 'vehicle_created_by')
                ->with('model.category', 'model.category.vehicleType')
            ...
            DB::rollBack();
            return $this->errorResponse('Booking failed: ' . $e->getMessage());
        }
        $setting = Setting::first();
        ...
        try {
            ...
            DB::rollBack();
            // Delete the created pending booking to maintain database consistency
            RentalBooking::where('booking_id', $rentalBooking->booking_id)->delete();
            return $this->errorResponse('Transaction failed, booking cancelled: ' . $e->getMessage());
        }
    ...
    public function iciciPaymentCallback(Request $request)
    {
        ...
        $bookingId = $request->addlParam1 ?? '';
        $paymentId = $request->addlParam2 ?? '';
        $txnID = $request->txnID ?? '';
        $booking = RentalBooking::select('booking_id', 'customer_id')->where('booking_id', $bookingId)->first();
        $customer = Customer::select('customer_id', 'is_test_user')->where('customer_id', $booking->customer_id)->first();
    ...
    public function uploadImages(Request $request, $booking_id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'images.*' => 'nullable|mimetypes:image/heic,image/heif,image/jpeg,image/png,image/jpg,image/bmp,image/gif,image/svg,image/webp|max:10000',
            'kilometers' => 'required|integer|min:1|regex:/^\d{1,7}$/',
    ...
    public function verifyStartJourney(Request $request, $bookingId)
    {
        ...
        $booking = RentalBooking::select('booking_id', 'return_date', 'start_otp', 'status')->where('booking_id', $bookingId)->first();
        if (!$booking) {
    ...
    public function getPricingShowCase(Request $request)
    {
        ...
        $vehicleId = $request->vehicle_id;
        $vehicle = Vehicle::select('vehicle_id', 'model_id', 'rental_price')->with('model.category.vehicleType')->where('vehicle_id', $vehicleId)->first();
        if (!$vehicle) {
